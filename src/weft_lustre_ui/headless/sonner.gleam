//// Headless sonner-compatible wrappers backed by toast primitives.

import weft_lustre
import weft_lustre_ui/headless/toast

/// Headless sonner toast corner.
pub type SonnerCorner =
  toast.ToastCorner

/// Headless sonner toast configuration.
...
/// Render headless sonner toast item.
pub fn sonner(
  config config: SonnerConfig(msg),
  content content: weft_lustre.Element(msg),
) -> weft_lustre.Element(msg) {
  toast.toast(config: config, content: content)
}

/// Render headless sonner region.
pub fn sonner_region(
  corner corner: SonnerCorner,
  children children: List(weft_lustre.Element(msg)),
) -> weft_lustre.Element(msg) {
  toast.toast_region(corner: corner, children: children)
}
