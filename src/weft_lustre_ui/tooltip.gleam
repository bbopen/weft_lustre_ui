//// Tooltip component built on the anchored overlay micro-solver (themed).
////
...
//// - applies theme-driven surface styling
...
fn base_styles(t: theme.Theme) -> List(weft.Attribute) {
  let #(bg, fg) = theme.overlay_surface(t)
...
pub fn tooltip_overlay(
  ...
  case config {
    TooltipConfig(key:, attrs: extra, ..) -> {
      ...
      let overlay_attrs =
        [
          overlay.overlay_root(key: key),
          positioned,
          weft_lustre.styles(base_styles(theme)),
          ..attrs
        ]
        |> list.append(extra)
